#!/usr/bin/env python3
"""
LEO macOS — Gercek zamanli sesli yardimci cekirdegi
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        "name": "get_weather",
        "description": (
            "Anlik hava durumunu ozetler. Varsayilan konum Lezhe, Arnavutluk'tur. "
...
def load_system_prompt() -> str:
    try:
        ...
        return (
            "Sen LEO'sun — macOS'ta çalışan kişisel AI asistanı. Adın LEO. "
...
class JarvisLive:
    ...
    def _on_text_command(self, text: str):
        ...
        if not self._loop or not self.session:
            self.ui.write_log("ERR: LEO bağlantısı henüz hazır değil.")
            return
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            time_ctx = f"[ŞU ANKİ ZAMAN — Europe/Tirane]\n{now.strftime('%A, %d %B %Y — %H:%M')}\n\n"
        except Exception:
    ...
    async def _listen_audio(self):
        ...
        try:
            ...
            print(f"[LEO] ❌ Mikrofon: {e}")
            raise
        finally:
            stream.close()

    async def _receive_audio(self):
        print("[LEO] 👂 Alım başladı")
        out_buf, in_buf = [], []
        output_noise = False
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if full_out:
                                ...
                                self.ui.write_log("ERR: LEO sesli yanıtını çözümlerken bir hata oluştu.")
                                if output_noise_samples:
                                    self.ui.write_debug(
        ...
        while True:
            ...
            try:
                ...
                async with (
                    ...
                    print("[LEO] ✅ Bağlandı.")
                    self.ui.set_state("LISTENING")
                    self.ui.write_log("SYS: LEO hazır. Dinliyorum...")
                ...
                traceback.print_exc()
                self.set_speaking(False)
                self.ui.write_log(f"ERR: LEO baglantisi kesildi veya internete ulasilamiyor — {e}")
                self.ui.set_state("ERROR")
                print("[LEO] 🔄 3 saniyede yeniden bağlanıyor...")
                await asyncio.sleep(3)


def main():
    if os.environ.get("TERM_PROGRAM") == "vscode":
        print("[LEO] VS Code icinden baslatildi.")
